...
infosoftglobal.FusionCharts.prototype = {
	...
	getSWFHTML: function() {
		...
		if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) { 
			...
			swfNode = '<embed type="application/x-shockwave-flash" src="'+ this.getAttribute('swf') +'" wmode="opaque" width="'+ this.getAttribute('width') +'" height="'+ this.getAttribute('height') +'"  ';
			swfNode += ' id="'+ this.getAttribute('id') +'" name="'+ this.getAttribute('id') +'" ';
			var params = this.getParams();
			 for(var key in params){ swfNode += [key] +'="'+ params[key] +'" '; }
			var pairs = this.getVariablePairs().join("&");
			 if (pairs.length > 0){ swfNode += 'flashvars="wmode=opaque&'+ pairs +'"'; }
			swfNode += '/>';
		} else { // PC IE			
			...
			var pairs = this.getVariablePairs().join("&");			
			if(pairs.length > 0) {swfNode += '<param name="flashvars" value="wmode=opaque&'+ pairs +'" />';}
			swfNode += "</object>";
		}
		return swfNode;
	},
